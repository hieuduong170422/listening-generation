...
def get_api_key_client() -> genai.Client:
    ...
    `vertexai=False` tường minh để bypass env GOOGLE_GENAI_USE_VERTEXAI — nếu không
    google-genai sẽ route client API-key sang aiplatform.googleapis.com (Vertex) và fail
    vì Vertex cần OAuth chứ không nhận API key.
    """
    api_key = get_api_key()
    if not api_key:
